...
def _generate_sas_url(blob_name: str, expiry_minutes: int = 60) -> str:
    ...
    return f"{ACCOUNT_URL}/{CONTAINER}/{blob_name}" f"?{sas_token}"
...
# ── Endpoints ─────────────────────────────────────────────────────────────────
@router.post("/{patient_id}/records", response_model=MedicalRecordOut, status_code=201)
async def upload_record(
    patient_id: str, file: UploadFile = File(...), db: Session = Depends(get_db)
):
    """Upload a medical record file to Azure Blob Storage."""
    # Validate patient
    patient = db.scalars(
        select(Patient).where(
            Patient.patient_id == patient_id, Patient.is_active.is_(True)
        )
    ).first()
    if not patient:
    ...
    if file.content_type not in ALLOWED_TYPES:
        raise HTTPException(
            400, f"File type {file.content_type!r} not allowed. Use PDF, JPEG or PNG."
        )
...
@router.get(
    "/{patient_id}/records/{record_id}/download", response_model=RecordDownloadOut
)
def download_record(patient_id: str, record_id: str, db: Session = Depends(get_db)):
    ...
    record = db.scalars(
        select(MedicalRecord).where(
            MedicalRecord.record_id == record_id, MedicalRecord.patient_id == patient.id
        )
    ).first()
