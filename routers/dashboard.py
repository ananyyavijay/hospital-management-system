...
def get_dashboard(patient_id: str, db: Session = Depends(get_db)):
    ...
    for appt in appointments:
        doctor = db.scalar(select(Doctor).where(Doctor.doctor_id == appt.doctor_id))
    ...
    return PatientDashboardResponse(
        patient=DashboardPatientInfo.model_validate(patient),
        upcoming_appointments=upcoming_appointments,
        past_appointments=past_appointments,
        medical_records=[
            DashboardRecord.model_validate(record) for record in medical_records
        ],
        stats=stats,
    )
