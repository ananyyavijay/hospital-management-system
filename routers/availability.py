...
def set_availability(
    doctor_id: str, data: SetAvailabilityRequest, db: Session = Depends(get_db)
):
    ...
    for slot in availability[data.date]:
        try:
            old_time = datetime.strptime(slot, "%I:%M %p").strftime("%H:%M")
    ...
    sorted_slots = sorted(existing_slots, key=lambda x: datetime.strptime(x, "%H:%M"))

    # Convert to AM/PM
    formatted_slots = [
        datetime.strptime(slot, "%H:%M").strftime("%I:%M %p") for slot in sorted_slots
    ]
...
def get_availability(doctor_id: str, db: Session = Depends(get_db)):
    ...
    return {"doctor_id": doctor.doctor_id, "availability": doctor.availability or {}}


@router.get("/{doctor_id}/availability/{date}")
def get_availability_for_date(doctor_id: str, date: str, db: Session = Depends(get_db)):
    if not re.fullmatch(r"\d{4}-\d{2}-\d{2}", date):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Date must be YYYY-MM-DD"
        )
    ...
    return {"doctor_id": doctor.doctor_id, "date": date, "slots": availability[date]}
...
def remove_availability(
    doctor_id: str, data: RemoveAvailabilityRequest, db: Session = Depends(get_db)
):
    doctor = db.scalar(select(Doctor).where(Doctor.doctor_id == doctor_id))
    ...
    for slot in availability[data.date]:
        try:
            converted = datetime.strptime(slot, "%I:%M %p").strftime("%H:%M")
    ...
    updated_slots = [slot for slot in existing_slots if slot not in data.slots]

    # Sort remaining slots
    updated_slots = sorted(updated_slots, key=lambda x: datetime.strptime(x, "%H:%M"))

    # Convert back to AM/PM
    formatted_slots = [
        datetime.strptime(slot, "%H:%M").strftime("%I:%M %p") for slot in updated_slots
    ]
