...
def test_patient_cannot_access_admin_patients(client):
    token = get_auth_token(client, "patient403@gmail.com", "test123", "patient")

    response = client.get(
        "admin/admin/patients", headers={"Authorization": f"Bearer {token}"}
    )
...
def test_admin_can_access_admin_patients(client):
    token = get_auth_token(client, "admin403@gmail.com", "test123", "admin")
